---
title: "11_2homework"
author: "Wenhao Hong, Feng Wang, Feixiang Liu, Renjun Luo"
date: "2022/9/27"
output:
...
Use imagematrix to display images (including original images, functions after cumulative distribution processing, logarithmic and normalized images)
...
```
As you can see from the image above, most of the data is clustered in a smaller area of x, resulting in a poor display of the first half of the dense distribution. So we change the x-axis range and output again. In addition, we modified the bin of the histogram to make it appear more clearly.

Plot a histogram of the distribution of your data and use different intervals to display the results. Here the data is processed.
Histogram of the distribution of the raw data
...
From the above formula, we can get a moment estimate of the parameters.
...
\end{cases}
$$
According to the above equation, we can get an estimate of the parameters.
...
According to the formula, define the method by which the moment estimator and the maximum likelihood estimate get the parameters.
...
Use the moment estimator and the maximum likelihood estimator to obtain the corresponding parameters to fit the curve.
...
ggplot(data=vector.bright, aes(x=UHV)) +
  ...
  xlim(0,2e6)
```
The red line is the result of using the SAR Gamma estimator.
The green line is the result of a combined moment estimate using the G0 distribution.
The blue line is the result of using a combination of the G0 distribution and the maximum likelihood estimate.